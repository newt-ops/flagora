...
export function ShopScreen({ profile, sessionToken }: ShopScreenProps) {
  ...
  return (
    <div className="flex w-full max-w-md mx-auto flex-col gap-4 text-tg-text pb-20">
      ...
      <div className="grid grid-cols-3 gap-1.5 rounded-2xl bg-tg-section p-1.5 shadow-sm">
        {categories.map((cat) => {
          const Icon = cat.icon;
        ...
        <div className="flex flex-col gap-3">
          {currentCategoryItems.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`flex flex-col rounded-2xl bg-tg-section p-4 shadow-sm transition-all ${
